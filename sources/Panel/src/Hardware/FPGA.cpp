...
void FPGA::Update() //-V2008
{
    ...
    {
        if (Channel::Current().typeMeasure.IsDuration() && ModeDuration::Current().IsNdt_1ns())
        {
            ReadInterpolator();
        }
        else if(Channel::Current().typeMeasure.IsDuration() && (ModeDuration::Current().IsFillFactor() || ModeDuration::Current().IsPhase()))
        {
            ReadFillFactorPhase();
        }
...
void FPGA::WriteDataGovernor()
{
    ...
    if (Read_READY != 0)                // \todo   сожалению, флаг готовности не работает так, как надо и если ожидать его установки в ноль, то происходит сбой передачи данных
...
{
    while (Read_READY != 0)             // \todo ѕровеерить. ¬озможно, по аналогии с передачей данных нельз€ ожидать флага готовности
    {
    }
