...
{
    SendCommand(COM_COL_ADDRESS, 28, 90);   // Устанавливаем адреса начального и конечного столбца. Первое число задает начальный адрес, второе - конечный адрес.
    SendCommand(COM_ROW_ADDRESS, 1, 63);    // Устанавливает адреса начальной и конечной строки. Первое число задает начальный адрес, второе - конечный адрес.
    ...
    for (int row = 0; row < HEIGHT_BUFFER - 1; row++)
    {
        ...
        {
            line[i / 2] = (uint8)(front[row][i] | (front[row][i + 1] << 4));
        }
