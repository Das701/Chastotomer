...
Global variables  
*********************************************************************************/
//uint8_t buf[20];              //SPI command buffer
//uint8_t buffer1[128];         //SPI data buffer    
//const uint8_t lookupval[4] = {0x0,0xF,0xF0,0xFF}; //lookup table
...
Description: send command via SPI
*********************************************************************************/
void Command(uint8 command)
{
    Delay(100);  //waits until SPI buffer is empty
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_5, GPIO_PIN_RESET); //set D/C# pin low
    //R_RSPI0_Send(buf,1);   
    HAL::SPI::Send(&command, 1); //send buffer via SPI
}
/*********************************************************************************
Function name: data
...
void Data(uint8 data)
{
    ...
    HAL::SPI::Send(&data, 1);     //send buffer via SPI
}
