...
#include "Display/Font/BigFont.h"
...
static const uint32_t	BigCharRow[] = {
	...
	0x007ff800,		// 57	00000000'01111111'11111110'00000000		9-14-9		(0235689)
	...
	0x0f1003e0,		// 89	00001111'10000000'00000011'11100000		4-5-13-5-5	(4)
	...
	0x007c0000,		// 98	00011111'00000000'00000000'00000000		3-5-24		(,)
	...
	0x1f8003e0,		// 103	00011111'10000000'00000001'11110000		3-6-14-5-4	(5)
...
/*!*****************************************************************************
  @brief	Big Text String print without Background Padding
  @param	disp - pointer to top left corner in LCD buffer memory
  @param	text - pointer to text string
  @param	palette - palette for background | foreground (lower 2 of 4 bytes)
  @param	monospace: true - font with the same character width
  @return
  */
void FontBig::BigString_print(char *text, int x, int y, Color color, bool monospace)
{
	while (*text) {
		uint32_t space = BigChar_print(*text, x, y, color);
		x += (monospace ? BIGCHAR_WIDTH : space);
		text++;
	}
}

/*!*****************************************************************************
  @brief	Big Text String print with Background Padding
  @param	disp - pointer to top left corner in LCD buffer memory
  @param	text - pointer to text string
  @param	palette - palette for background | foreground (lower 2 of 4 bytes)
  @param	monospace: true - font with the same character width
  @return
  */
void FontBig::BigStringPad_print(char *text, uint8 *disp, uint32_t palet, bool monospace)
{
	while (*text) {
		uint32_t space = BigCharPad_print(*text, disp, palet);
		disp += (monospace ? BIGCHAR_WIDTH : space);
		text++;
	}
}
...
/*!*****************************************************************************
  @brief	Big Character print without background padding
  @param	symb - character
  @param	disp - pointer to top left corner in LCD buffer memory
  @param	palet - palette for (background |) foreground (lower (2) 1 of 4 bytes)
  @return	width of character in pixels (for proportional string printing)
  */
uint32_t FontBig::BigChar_print(uint8_t symb, int x, int y, Color color)
{
	uint16_t offs = BigCharOffs[symb - ' '];	// offset in BigCharIndx
	const uint8_t *indx = (uint8_t *)&BigCharIndx + offs;	// first row index

	uint32_t width = 0;
	if (symb == ' ') width = BIGSPACE_WIDTH;		// space width

	while (*indx) {
        
		uint32_t same = 1;						// number of the same rows
		uint32_t code = *indx++;
		if (code < BIGSTART_INDEX) same = code;
		else {
			uint32_t row = BigCharRow[code - BIGSTART_INDEX];
			for ( ; same-- ; ) {
				for (uint32_t i = 0; i < 32; i++) {
					uint32_t pixel = row & 0x80000000;
...
/*!*****************************************************************************
  @brief	Big Character print with background padding
  @param	symb - character
  @param	disp - pointer to top left corner in LCD buffer memory
  @param	palet - palette for background | foreground (lower 2 of 4 bytes)
  @return	width of character in pixels (for proportional string printing)
  */
uint32_t FontBig::BigCharPad_print(uint8_t symb, volatile uint8_t *disp, uint32_t palet)
{
	uint16_t offs = BigCharOffs[symb - ' '];	// offset in BigCharIndx
	const uint8_t *indx = (uint8_t *)&BigCharIndx + offs;	// first row index
